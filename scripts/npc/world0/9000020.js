...
function action(mode, type, selection) {
	if (mode == 1)
		...
		if (status == 3 && mode == 0)
			cm.sendNext("Please don't waste my time, I'm busy.");
		cm.dispose();
		return;
	}
	
	if (status == 0) {
		cm.sendNext("#eHello, I'm in charge of transportation in #dSharpStory#k!");
	} if (status == 1) {
		...
		cm.sendYesNo("#eDo you want to go to #r#m" + chosenMap + "##k?");
	} else if (status == 4) {
		cm.warp(chosenMap, 0);
		cm.dispose();
	}
