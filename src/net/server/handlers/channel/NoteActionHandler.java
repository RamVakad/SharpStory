    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (action == 0 && c.getPlayer().getCashShop().getAvailableNotes() > 0) {
            ...
            for (int i = 0; i < num; i++) {
                ...
                //Stupiddd, sooo stupidd!
                PreparedStatement ps;
                try {
                    ps = DatabaseConnection.getConnection().prepareStatement("SELECT `fame` FROM notes WHERE id=? AND deleted=0 AND checked=0");
                    ps.setInt(1, id);
                    ...
                    ps = DatabaseConnection.getConnection().prepareStatement("UPDATE notes SET `checked` = 1 WHERE id = ?");
                    ps.setInt(1, id);
                    ps.executeUpdate();
                    ps.close();
                } catch (Exception e) {
                    System.out.println("Exception at NoteActionHandler.java: " + e);
                }
            }
            if (fame > 0) {
